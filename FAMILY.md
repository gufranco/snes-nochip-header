...
**Type checking is half finished.** `pyproject.toml` carries the strict mypy
configuration the family uses, and `mypy` currently reports about 105 findings,
all of them in `*.test.py` files and nearly all of them missing annotations. The
source modules are clean. The CI job that runs mypy is deliberately absent until
the count reaches zero; adding it back is the last step, not the first.

The mechanical part is done. What is left is annotating test helpers, and two
classes of finding that are not mechanical:

- `Module "romimage.dump" does not explicitly export attribute "..."`. A test
  reaching through one module to a name another module defines. Import it from
  where it is defined rather than adding it to an `__all__` that does not want it.
- `Cannot determine type of "found"`. A name a test binds in one branch and reads
  in another.

**No `specs/current/`.** Every sibling carries requirements with Given/When/Then
scenarios. This one does not.

**No `conformance/hardware.json`.** The header fields this project rewrites are
documented in the Nintendo development manual, and nothing here quotes it.
